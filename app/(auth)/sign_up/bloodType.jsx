...
const BloodType = ({ type, onPress, isSelected }) => {
  return (
    <View>
      ...
      >
        <Text
          style={[isSelected && styles.txtColor]}
          className="text-2xl font-bold text-primary_red"
        >
          {type}
        </Text>
      </TouchableOpacity>
...
const BloodGroup = () => {
  const [bloodType, getSelectedType] = useState(null);
  ...
  return (
    <View className="bg-white h-full">
      <SignUpHeader text={`What's your Blood Type?`} />
      <View className="w-[90%] mx-auto my-2">
        <Text className="text-base text-[#B8B8B8] text-center">
          If you Don't know your blood type just, select any for now.
        </Text>
      </View>
      ...
      {error && (
        <Text className="text-center py-5 text-primary_red text-lg font-bold">
          You must select a blood group to proceed
        </Text>
      )}

      <ThemeButton title={"Continue"} onPress={handlePressed} />
    </View>
  );
};
