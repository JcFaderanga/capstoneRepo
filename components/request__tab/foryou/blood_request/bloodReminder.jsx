...
const bloodReminder = ({blood_type}) => {
  const {user} = useAuth();

 const compatibility = ShowCompatibility(blood_type? blood_type : user.blood_type)
 const {canDonateTo, canReceiveFrom} = compatibility;

  return (
    <View className="w-full border-b border-gray-200 py-3">
        <Text className='text-lg '>
            {blood_type ? 'Blood type ' : 'Your blood type '}
            <Text className="text-primary_red font-bold"> {blood_type? blood_type :user.blood_type} </Text> 
