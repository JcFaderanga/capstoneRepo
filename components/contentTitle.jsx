...
const ContentTitleButton = ({title, icon,backIcon, onPress, size}) => {
  return (
    <TouchableOpacity className=" flex-row justify-between items-center w-full h-20 border-b-2 border-[#F2F2F2] bg-white"
            onPress={onPress}>      
            ...
            <View className="w-20 h-full flex justify-center items-center">
                <Image source={backIcon} className="h-5 w-6" resizeMode='contain'/>
            </View>
    </TouchableOpacity>
  )
}
